...
def build_header(feature, qualifier_list):
    ...
    for item in qualifier_list:
        if item == 'location':
            ...
            if item not in feature.qualifiers.keys() and item == 'gene':
            # if not feature.qualifiers.has_key(item) and item == 'gene':
                ...
                # if feature.qualifiers.has_key('locus_tag'):
                    ...
                    item = 'db_xref'
            elif item not in feature.qualifiers.keys() and item == "locus_tag":
            ...
            if item in feature.qualifiers.keys():
    ...
    for feature in record.features:
        if feature.type == feature_type: # What kind of feature to extract. Usually CDS or tRFLP
            if sequence_type == 'nt':
                ...
                id = build_header(feature, qualifier_list), description = record.id)
...
#### Main ####
##############
in_file_handle = open (in_file, 'r', newline=None) #The 'U' option so we don't have to worry about line endings
